...
const ScrollingGallerySection: React.FC<ScrollingGallerySectionProps> = ({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const cols = gsap.utils.toArray<HTMLElement>(".col", sectionRef.current);
      ...
      cols.forEach((col, i) => {
        ...
        items.forEach((item) => {
          const columnHeight =
            (item as HTMLElement).parentElement!.clientHeight;
      ...
      // ScrollTrigger limitado a ESTA sección
  ...
  return (
    <section
      ref={sectionRef}
      className="relative min-h-[500vh] flex flex-col justify-center bg-[#eee] overflow-hidden"
    >
        ...
        {images.map((colImages, colIndex) => (
          <div key={colIndex} className="col flex flex-col flex-1">
            {colImages.map((src, i) => (
              <div
                key={i}
                className="image p-4 filter saturate-0 hover:saturate-100"
              >
                <img
                  src={src}
