...
const useWhatsApp = (product: Product | null) => {
  const phoneNumber = '5493492524734';
...
const ProductDetail: React.FC = () => {
  ...
  const handleWhatsAppClick = (useWeb: boolean) => {
    if (!product) {
      alert('Error: No se ha cargado la información del producto.');
      return;
    }
  ...
  if (!product) {
    return <div className="text-center py-4">No se pudo cargar el producto.</div>;
  }
