...
interface Order {
  ...
  const SalesDashboard: React.FC<SalesDashboardProps> = ({
    ...
    const filterOrders = (period: FilterPeriod) => {
      ...
      const filtered = orders.filter(order => new Date(order.date) >= startDate);
    ...
    return (
      <main className="grid flex-1 ml-16 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
        <div className="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-2">
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>This {filterPeriod.charAt(0).toUpperCase() + filterPeriod.slice(1)}</CardDescription>
          ...
          <Tabs defaultValue={filterPeriod} onValueChange={(value) => setFilterPeriod(value as FilterPeriod)}>
